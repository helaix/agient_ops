...
1. **Hierarchical Delegation**: Tasks flow from parent agents to child agents in a structured manner
2. **Consistent Reporting**: Regular updates flow from child agents to parent agents
3. **Unified Code Management**: All code changes are consolidated into a single branch managed by the top-level agent
4. **Information Synthesis**: Each level of agents synthesizes information from their child agents before reporting upward
...
2. **Level 1 Agents**:
   ...
   - Monitor progress of child agents
...
2. **Feedback and Guidance**:
   ...
   - Offer guidance when child agents encounter challenges
...
3. **Code Integration Flow**:
   ...
   - Child agents push to their branches
...
4. **PR Creation Rules**:
   ...
   - Child agents should never create PRs directly to main
   - Child agents should push to their assigned branches
...
### 1. Blocked Child Agent

**Scenario**: A Level 2 Agent is blocked and cannot proceed with their task.

**Resolution Process**:
1. Child Agent immediately reports the blocker to their parent agent
...
4. Once resolved, Child Agent continues with implementation
...
2. Parent Agent determines the best resolution approach:
   ...
   - For complex conflicts, consult with relevant Child Agents
